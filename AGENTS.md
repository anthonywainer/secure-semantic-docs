...
- Follow PEP 8.
- Always format code before committing. Use `ruff format` to auto-format and `ruff check --fix` to auto-fix lint
  violations:
  ```bash
  ruff format .
  ruff check --fix .
  ```
  Ruff is PEP 8-compliant and replaces `black`, `isort`, and `flake8` in a single tool. Add it to dev dependencies
  and configure a minimal `[tool.ruff]` section in `pyproject.toml`.
- Never submit code that fails `ruff check .` or diverges from `ruff format` output.
