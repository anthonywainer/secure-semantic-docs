"""DDL-based schema loader for lakehouse layers.

Schemas are stored as SQL DDL files under::

    src/secure_semantic_docs/resources/schemas/<schema>.<table>.ddl

File naming mirrors the Iceberg table layout:

    bronze.documents.ddl   -- bronze layer  (CREATE TABLE bronze.documents)
    silver.chunks.ddl      -- silver layer  (CREATE TABLE silver.chunks)
    gold.embeddings.ddl    -- gold layer    (CREATE TABLE gold.embeddings)

Callers use the logical name (e.g. ``"bronze_documents"``) which is
translated to the correct file via :data:`_SCHEMA_FILE_MAP`.
...
_SCHEMAS_DIR = Path(__file__).resolve().parent.parent / "resources" / "schemas"

# Maps the logical table key used throughout the codebase to the DDL file name.
# File names mirror the Iceberg schema.table convention.
_SCHEMA_FILE_MAP: dict[str, str] = {
    "bronze_documents": "bronze.documents.ddl",
    "silver_chunks": "silver.chunks.ddl",
    "gold_embeddings": "gold.embeddings.ddl"
}
...
def load_schema(table_name: str) -> StructType:
    ...
    Reads the DDL file mapped from *table_name* in :data:`_SCHEMA_FILE_MAP`
    and converts it to a :class:`StructType` via :func:`StructType.fromDDL`.

    Parameters
    ----------
    table_name:
        Logical table key: ``"bronze_documents"``, ``"silver_chunks"``,
        or ``"gold_embeddings"``.
